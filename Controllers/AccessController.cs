                ...
                if (usersAttributes != null)
                {
                    ...
                    if (responseXml.Result == false)
                    {
                        await _logFunction.LogModelDecisions(new Evaluation_results
                        {
                            ...
                            xacml_threshold = (float) formulaConstants.xacmlTrustThreshold,
                            xacml_constant = (float) formulaConstants.xacmlConstant,
                            ngac_constant = (float) formulaConstants.NGACConstant,
                            xacml_result = false,
